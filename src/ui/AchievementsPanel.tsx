      ...
      <div className="fixed top-14 right-4 z-[100] flex flex-col gap-2 pointer-events-none">
        {toasts.map(t => (
          ...
          >
            ...
            <div>
              <div className="text-amber-200 font-bold text-sm">{t.achievement.name}</div>
              <div className="text-amber-400/80 text-xs">{t.achievement.description}</div>
            </div>
          </div>
        ))}
      </div>

      {/* Achievement panel — bottom left */}
        ...
        {expanded && (
          <div className="mb-2 w-64 bg-slate-900/95 border border-slate-700 rounded-lg shadow-xl backdrop-blur-sm max-h-80 overflow-y-auto">
            <div className="sticky top-0 bg-slate-900/95 px-3 py-2 border-b border-slate-700">
              <div className="text-slate-300 font-bold text-sm">Achievements</div>
              <div className="text-slate-500 text-xs">{unlocked} / {total} unlocked</div>
            </div>
              ...
              {achievements.map(a => (
                <div
                  ...
                  className={`flex items-start gap-2 px-2 py-1.5 rounded transition-colors ${
                    a.unlocked ? 'bg-amber-900/20 border border-amber-800/40' : 'opacity-40'
                  }`}
                >
                  <span className="text-sm mt-0.5">{a.unlocked ? '🏆' : '🔒'}</span>
                  <div className="min-w-0">
                    <div className={`text-xs font-bold leading-snug ${a.unlocked ? 'text-amber-200' : 'text-slate-500'}`}>
                      {a.name}
                    </div>
                    <div className={`text-xs leading-snug ${a.unlocked ? 'text-slate-400' : 'text-slate-600'}`}>
                      {a.description}
                    </div>
                  </div>
                </div>
