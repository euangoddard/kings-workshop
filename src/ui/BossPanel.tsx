...
export default function BossPanel() {
  ...
  const hpRatio = bossActive ? Math.max(0, bossHPVal.div(bossMaxHP).toNumber()) : 1
    ...
    >
      <div className="w-full max-w-2xl flex flex-col items-center gap-1.5">
        ...
        <div className="flex items-center gap-3 w-full justify-between">
          <div className="text-slate-600 text-xs tracking-widest uppercase">Battlefield</div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className={`font-bold text-sm ${elite ? 'text-amber-400' : 'text-amber-400'}`}>
              Boss #{currentBoss}
            </div>
          </div>
        ...
        {bossActive ? (
          <>
            ...
            <div className="flex items-center gap-4 w-full justify-between">
              <div className={`text-xs font-bold ${elite ? 'text-amber-400' : 'text-red-400'}`}>
            ...
            >
              ⚔️ Fight! <span className="text-red-300 text-xs font-normal">(+{fmt(clickDmg)} bonus dmg)</span>
            </button>
          </>
        ) : (
          <>
            ...
            <button
              ...
              `}
            >
              {elite ? '⚡ Challenge Elite Boss #' : '⚔️ Challenge Boss #'}{currentBoss}
            </button>
          </>
        )}
      </div>
