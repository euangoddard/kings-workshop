        ...
        <div className="w-full max-w-2xl flex flex-col items-center gap-1.5">
          ...
          <div className="flex items-center gap-3 w-full justify-between">
            <div className="flex items-center gap-2">
              ...
              <div className={`font-bold text-sm ${elite ? "text-amber-400" : "text-red-400"} animate-pulse`}>
                ⚔️ Boss #{currentBoss} — {bossInfo.name}
              </div>
            </div>
            <div className="flex items-center gap-3 text-slate-500 text-xs">
              <span>Army DPS: {fmt(myDPS)}</span>
            </div>
          </div>
          ...
          <div className="w-full bg-slate-800 rounded-full h-4 border border-slate-700 overflow-hidden shadow-inner">
            <div
              className={`h-4 rounded-full transition-all duration-200 ${isDamaged ? "brightness-150" : ""}`}
              style={{
