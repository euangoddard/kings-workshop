...
export default function ArmouryPanel() {
  ...
  const troopCounts: Record<TroopType, number> = {
    infantry,
    archer: archers,
    cavalry,
    mage: mages,
  ...
  const troopUpgrades = {
    infantry: infantryUpgrades,
    archer: archerUpgrades,
    cavalry: cavalryUpgrades,
    mage: mageUpgrades,
  };
  const equipActions: Record<TroopType, () => void> = {
    infantry: equipInfantry,
    archer: equipArcher,
    cavalry: equipCavalry,
    mage: equipMage,
  };
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-1.5 bg-slate-900/70 border-b border-slate-700">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-400">
          ...
          <span>
            DPS: <span className="text-amber-400">{fmt(totalDPS)}</span>
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-1 min-h-0">
          ...
          {TROOPS.map((t) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`mt-0.5 ${isActive ? colorClass(t.color, "badge") : ""}`}
                >
                  {t.label}
                </span>
              </button>
        ...
        <div className="flex-1 flex flex-col gap-2 px-2 pt-4 pb-4 overflow-y-auto">
          {/* Troop header */}
          <div
            className={`w-full bg-slate-800/70 rounded-lg p-2.5 border ${colorClass(cfg.color, "borderDim")} text-center`}
          >
            <div
              className={`${colorClass(cfg.color, "text")} font-bold text-xl`}
            >
              {count}
            </div>
            ...
            <div className="text-slate-600 text-xs mt-0.5">
              DPS:{" "}
              <span className={colorClass(cfg.color, "textDim")}>
                {effectiveDPS}
              </span>
            </div>
          </div>
          ...
          >
            ...
            <div
              className={`text-xs mt-0.5 font-normal ${canEquip ? colorClass(cfg.color, "textDim") : "text-slate-600"}`}
            >
          ...
          {/* Upgrade branches */}
          <div className="text-slate-500 text-xs font-semibold uppercase tracking-widest pl-0.5">
            Upgrades
          </div>
          {BRANCHES.map((branch) => {
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1">
                    ...
                    <span
                      className={`text-xs font-bold ${colorClass(cfg.color, "text")}`}
                    >
